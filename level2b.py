...
def a_star(start, goal, time_limit, maze):
    ...
    frontier.put((start, 0), 0)  # (position, current_time)
    came_from = {}
    cost_so_far = {}
    ...
    while not frontier.empty():
        current, current_time = frontier.get()
        ...
        for next in get_neighbors(current, maze):
            ...
            new_time = current_time + wait_time(current, maze)
            
            if new_time <= time_limit and (next not in cost_so_far or new_cost < cost_so_far[next]):
                cost_so_far[next] = new_cost
                priority = new_cost + heuristic(next, goal)
                frontier.put((next, new_time), priority)
